...
// the radius is how far in either direction (higher than the average, or lower than the average)
// the result may be.
float rand_radius_retry(float average, float radius, float min, float max) {
    ...
    while (1) {
        ...
        if (retVal >= min && retVal <= max) return retVal;
    }
...
void test_random_functions(unsigned long numbersToGen) {
    ...
    printf(
        "Time for stdlib.h's rand() function to calculate %lu random numbers is %d s\n",
        numbersToGen,
        stdTime
    );
    ...
    printf(
        "Time for mt19937int.h's genrand() function to calculate %lu random numbers is %d s\n",
        numbersToGen,
        mtTime
    );
}
