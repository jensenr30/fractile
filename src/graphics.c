...
/// this returns the pixel data of a pixel at a certain point on a surface (color and alpha in an
/// Uint32)
...
// this is a bounded pixel set (it won't crash the program when it tells SDL to set a pixel outside
// of a surface
void draw_pixel(SDL_Surface *dest, int x, int y, Uint32 color) {
    if (x >= 0 && y >= 0 && x < dest->w && y < dest->h) set_pixel(dest, x, y, color);
}

/// this function will mix two colors (according to their respective weight) and return the
/// resulting color.
...
Uint32 color_mix_weighted(Uint32 color1, Uint32 color2, Uint32 weight1, Uint32 weight2) {
    ...
    Uint32 alpha =
        (((color1 >> 24) & 0xff) * weight1 + ((color2 >> 24) & 0xff) * weight2) / (weight);
...
Uint32
color_mix_weighted_f(unsigned int color1, unsigned int color2, float weight1, float weight2) {
...
// colorBorder and colorFill are interpreted as 32-bit colors (0xAARRGGBB) where A = alpha, R = red,
// B = blue, G = green. if doFill is 0, then this function will only print the border. if doFill is
// any non-zero number, then the rectangle will be filled with the "colorFill" color. returns 0
// success. returns 1 on invalid dest
short draw_rect(
    SDL_Surface *dest,
    int x,
    int y,
    ...
    for (i = istart; i < istop; i++) {
        for (j = jstart; j < jstop; j++) {
            ...
            if (fabs(i - x) < borderThickness || fabs((x + w) - i) <= borderThickness ||
                fabs(j - y) < borderThickness || fabs((y + h) - j) <= borderThickness) {
...
// returns 2		 if the lines are parallel and they are the same line (have all points in
// common)
...
int intersect_p(
    float x1, float y1, float slope1, float x2, float y2, float slope2, float *xr, float *yr
) {
    ...
    if (slope1 == slope2 || slope1 == -slope2) { // if the two lines share the same slope,
        ...
        float connSlope =
            (y2 - y1) /
            (x2 - x1
            ); // calculate the slope of a third line that connects the two given data points.
        if (connSlope == slope1 ||
            connSlope == -slope1) { // if the slope between the two points given in the same
                                    // magnitude as the slopes of the lines,
...
void draw_line(
    ...
    // these are the slopes of x and y. This describes how fast they change with respect to a unit
    // step in the line parameter, t.
    float xslope;
    float yslope;

    // this is the maximum difference (either the difference in y or the difference in y, depending
    // on which is bigger.)
    float bigdiff;
    ...
    if (xslope == 0) {         // make sure xslope is not zero. we need to divide by that!
        xslope = 1 / VERTICAL; // make xslope insanely tiny (so that y can change a lot and x will
                               // change change a VERY small amount.
    }
    //-------------------------------------------------------------------------------
    ...
    if (!within_screen(x1, y1) && !within_screen(x2, y2)) {
        ...
        // a temporary solution (this doesn't allow lines that have points outside the screen to be
        ...
        if (intersect_p(
                x1, y1, yslope / xslope, 0, get_window_height() - 1, HORIZONTAL, &xr, &yr
            ) == 1) {
            if (within_screen(xr, yr)) {
                if (foundOne == 0) {
                    xr1 = xr;
        ...
        if (intersect_p(x1, y1, yslope / xslope, get_window_width() - 1, 0, VERTICAL, &xr, &yr) ==
            1) {
            if (within_screen(xr, yr)) {
                if (foundOne == 0) {
                    xr1 = xr;
    ...
    //-------------------------------------------------------------------------------
        ...
        // this records the direction that x travels when going from inside the screen to outside
        // it. (1 or -1)
        ...
        // check right bound intersection
        if (!foundOne &&
            intersect_p(x1, y1, yslope / xslope, get_window_width() - 1, 0, VERTICAL, &xr, &yr) ==
                1) {
            if (within_screen(xr, yr)) {
                if ((xin - xr) * (xin - xout) >= 0) {
                    xr1 = xr;
        ...
        if (foundOne) {
            // draw a line between that exit point and the one valid point inside the screen. now no
            // part of the line is outside the screen.
            if (within_screen(x1, y1))
...
/// DON'T BOTHER TAKING THE SQUARE ROOT! JUST CHECK THE SQUARES! IT HAS THE SAME EFFECT! SQUARE THE
/// RADIUS AND THEN CHECK IF ( (i-x)*(i-x)+(j-y)*(j-y) ) is less than ( radius*radius )
