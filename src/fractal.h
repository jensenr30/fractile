...
The "graphics.c" and "graphics.h" files are utilized in order to draw lines, shapes, and other
things on the screen. These lines and shapes are the building blocks of the fractals drawn here.
...
#include <inttypes.h>
// I wish I didn't have to include this here, but I need it because SDL surfaces are passed through
// fractal functions.
#include "SDL2/SDL.h"
...
// children are the points that will be used as the origins when rendering he NEXT iteration of the
// fractal. consequently, this is how many copies
...
// this is how far away (in pixels) a user's click has to be from an object for it to count as being
// selected
...
#define fst_circle                                                                                 \
    2 // this draws a circle with center <x[0],y[0]> and a radius of the [radius] variable
#define fst_tri                                                                                    \
    3 // this uses <x[0],y[0]>, <x[1],y[1]>, and <x[2],y[2]> as points to specify a triangle.
#define fst_rect 4 // this uses <x[0],y[0]> and <x[1],y[1]> as opposite vertices of a rectangle

#define fst_TOP                                                                                    \
    4 // the highest valid fractal shape type (the last one). This is used when randomly selecting a
      // shape type.
...
struct fractalChild {
    ...
    // this is a number (in degrees) that specifies how much this child is rotated with respect to
    // its parent.
    float twist;
...
/// this structure is used to store which component of a fractal is selected and being graphically
/// adjusted (manually) by the user
// this keeps track of what part(s) of a fractal is being played with
// this structure is built in such a way as to allow the user to select multiple parts of a fractal
// at one time.
