...
// I supposed if you wanted to change font mid-way through the program you could call it a second
// time. it returns: 0	operation success -1	bad sb pointer -2	could not open font1 -3
// could not open font2
...
int sidebar_render(struct sidebar *sb, SDL_Surface *dest) {
    ...
    genrect.w = sb->topButtonsWidth - 2 * sb->buttonsBorder;
    genrect.h = sb->topButtonsHeight - 2 * sb->buttonsBorder;
    genrect.x = sb->topButtonsLeftRightWidth + 0 * sb->topButtonsWidth + sb->buttonsBorder +
                sb->font1OffsetX;
...
// Returns:
//	0	action was NOT relevant to the sidebar and it will need to be evaluated by something
// else. 	1	action WAS relevant to the sidebar and was "used" by it.
